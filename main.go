...
// Weather Create struct to hold info about weather
type Weather struct {
	Id        int
	Ok        bool
	Date      string
...
func main() {
	...
	av, err := dynamodbattribute.MarshalMap(weather)
	if err != nil {
		log.Fatalf("failed to DynamoDB marshal weather, %v", err)
	}

	tableName := "tf_km_demo_table"
